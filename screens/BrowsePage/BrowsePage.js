...
const BrowsePage = ({ navigation, route }) => {
  ...
  const sortByDate = () => setSortType(true);
  ...
  const createTwoButtonAlert = () =>
    Alert.alert("", 'במיון לפי מרחק, מוצגים כלבים במרחק של עד 10 ק"מ בלבד', [
      ...
      <Provider>
        ...
        >
          ...
          <View style={styles.sepreator}>
            <Text style={styles.seperateText}>/</Text>
          </View>
          ...
          <View position="absolute" marginLeft="80%">
            <TouchableOpacity
              title={"2-Button Alert"}
              onPress={createTwoButtonAlert}
            >
              <Icon name="infocirlce" size={22} color="#DCA277" />
            </TouchableOpacity>
          </View>
        </View>
        <View style={styles.listContainer}>
          {sortType && (
            <FlatList
              ...
              onEndReached={() =>
                getNextData(data, setData, collectionPath, initialLocation)
              }
          ...
          {!sortType && (
            <FlatList
              ...
              refreshControl={
                <RefreshControl
                  refreshing={refreshing}
                  onRefresh={refreshDistanceItems}
                />
              }
              numColumns={2}
              renderItem={({ item }) => {
