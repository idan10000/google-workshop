...
export default function Screen3Report({ route, navigation }) {
  ...
  const initSelectedTagList = route.params.edit
    ? report.tagList.map((tag) => ({ tag: tag, state: false }))
    : [];
  ...
  const nextScreen = async () => {
    if (
      ...
      const dbReport = new Report(
        ...
        user.uid,
        timeStamp
      ); // Report to upload to DB
      const sendReport = new Report(
        image,
        "",
        location,
        address,
        ...
        user.uid,
        timeStamp
      ); // Report to send to the Report page
      ...
      if (route.params.edit) {
        ...
        if (deepDiffer(sendReport, report)) {
          const docRef = await setDoc(
            doc(db, "Reports", route.params.ref).withConverter(reportConverter),
            dbReport
          )
            .then(() => {
        ...
        <ScrollView style={Nofar_styles.container}>
          <Provider>
            <View style={Nofar_styles.container}>
              ...
              >
                <Text style={Nofar_styles.TinyButtonTitle}>
                  {route.params.edit ? "שמירת שינויים" : "יצירת דיווח"}
                </Text>
              </TouchableOpacity>
            </View>
          </Provider>
