...
export default function MapForCreation({
    ...
    useEffect(async () => {
        if (usePreLocation) {
            ...
            let json = await Geocoder.from(preLocation.latitude, preLocation.longitude)
            ...
            await (async () => {
                ...
                let json = await Geocoder.from(newLocation.coords.latitude, newLocation.coords.longitude)
                let address = json.results[0].formatted_address
                setAddress(address.substring(0, address.length - 7))
                setPin({
            ...
            <View style={{flex: 1,
                ...
                <GooglePlacesAutocomplete
                    ...
                    onPress={async (data, details = null) => {
                        ...
                        let json = await Geocoder.from(newLocation.latitude, newLocation.longitude)
                        let address = json.results[0].formatted_address
                        setAddress(address.substring(0, address.length - 7))
                    }}
